...
public class OfficesDAO {
    ...
    public Office getOfficeByID(String officeCode) {
        ...
        try {
            ...
            // can be single RS pr 0 rows found;
            if (rs.next()) {
                office = new Office();
    ...
    public List<Office> getAllOffices() {
        ...
        try {
            Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/classicmodels", "root", "");
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM classicmodels.offices");

            // can be multiple roes OR can be single RS OR 0 rows found;
            while (rs.next()) {
                Office office = new Office();
                office = extractOffice(rs);
                offices.add(extractOffice(rs));
            }
            ;
        } catch (SQLException throwables) {
            throwables.printStackTrace();
    ...
    public List<Office> getAllOfficesByCountry(String country) {
        List<Office> offices = new ArrayList<>();
        try {
            Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/classicmodels", "root", "");
            Statement stmt = conn.createStatement();
    ...
    public void updateOffice(Office office) {
        ...
        try (Connection conn = DBUtils.getConnection()) {
            ...
            if (result == 1) {
                System.out.println("Office with ID = '" + office.getOfficeCode() + "' Updated Successfully ");
            } else {
    ...
    public void deleteOfficeByID(String officeCode) {
        ...
        try (Connection conn = DBUtils.getConnection()) {
            ...
            if (result == 1) {
                System.out.println("Office with ID = '" + officeCode + "' Deleted Successfully ");
            } else {
                System.out.println("Office NOT Removed with Code = '" + officeCode + "'");
            }
    ...
    public void createOffice(Office office) {
        String sql = "INSERT INTO classicmodels.offices (officeCode, city, phone, addressLine1, addressLine2, country, postalCode, territory) " +
        ...
        try (Connection conn = DBUtils.getConnection()) {
            ...
            int result = stmt.executeUpdate(sql);
            if (result == 1) {
                System.out.println("Office with ID = '" + office.getOfficeCode() + "' Added Successfully ");
            } else {
                System.out.println("Office NOT Added with Code = '" + office.getOfficeCode() + "'");
            }
